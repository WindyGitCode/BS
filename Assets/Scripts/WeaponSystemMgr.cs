...
public class WeaponSystemMgr : Singleton<WeaponSystemMgr>
{
    ...
    private void InitWeaponData()
    {
        ...
        {
            AddWeapon(E_Weapon.weapon_knife, 3 , "匕首", 0, "Controller/RoleGamingController/Knife_Controller",
                      "Effects/KnifeHit", "Audio/KnifeHit", 0, 0, true);
            AddWeapon(E_Weapon.weapon_handgun, 1,"沙漠之鹰", 500, "Controller/RoleGamingController/Handgun_Controller",
                      "Effects/GunFire", "Audio/Weapon/HandgunFire", 20, 0.2f, false);
            ...
            AddWeapon(E_Weapon.Grenade,4, "破片手雷", 300, "Controller/RoleGamingController/Grenade_Controller",
    ...
    { 
        if (weaponDict.TryGetValue(weaponType, out WeaponData weapon))
        {
            ...
            if (PlayerMgr.Instance.playerData.money < weapon.price)
            {
                Debug.Log($"金币不足，无法购买{weapon.weaponName}（需{weapon.price}，当前{PlayerMgr.Instance.playerData.money}）");
                return false;
            }
            // 扣除金币+标记解锁
            PlayerMgr.Instance.playerData.money -= weapon.price;
            weapon.isUnlocked = true;
            // 保存玩家数据
            JsonMgr.Instance.SaveData(PlayerMgr.Instance.playerData, "PlayerData");
            Debug.Log($"成功购买{weapon.weaponName}，剩余金币：{PlayerMgr.Instance.playerData.money}");
            return true;
        }
